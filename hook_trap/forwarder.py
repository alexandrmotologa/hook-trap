...
# Headers that should not be copied directly to the target request
...
async def forward_request(
    ...
    for k, v in req_detail.get("headers", {}).items():
        if k.lower() not in FILTERED_FORWARD_HEADERS:
            headers_to_send[k] = v
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
            ...
            # Capture response text up to 64KB
    ...
    # Broadcast replay event to live viewers
